...
import {Button, Card, Carousel, Empty, Image} from "antd";
...
export function Birthday(): JSX.Element {
  ...
  const data = dataBirthday?.data
    ? dataBirthday?.data?.length < 5
      ? [1, 2, 3, 4]
      : dataBirthday?.data
    : [1, 2, 3, 4];
  ...
  return (
    <div className="container-birthday">
      <div className="flex">
        ...
        <Button
          type="primary"
          className="bg-blue-500 ml-6"
          onClick={() => {
            setIsList(!isList);
          }}
        >
          {isList ? "View list" : "View Carousel"}
        </Button>
      </div>
      {isList ? (
          ...
          >
            {data?.map((item, index) => (
              <div key={index} className="w-[40%]">
                <img
                  src="img/birthday/bg-birthday.png"
                  key={index}
                  alt="swipe"
                />
                <img
                  src={
                    dataBirthday.data && dataBirthday.data[index]?.avatar
          ...
          {dataBirthday.data && dataBirthday.data?.length > 0 ? (
            dataBirthday.data?.map((item, index) => (
              <div className="p-5" key={index}>
                ...
                >
                  <Card.Meta
                    className="birthday-text row-all-center"
                    title={item.fullName}
                    description={
                      moment(item.dateOfBirth).format("DD-MM") +
                      "-" +
                      filterYear
                    }
                  />
